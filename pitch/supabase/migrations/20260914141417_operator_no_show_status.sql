-- Add true no-show lifecycle support for the ResyOS-style iPad demo.
...
alter table reservation_demo.bookings
  ...
  check (status in ('confirmed','checked_in','seated','completed','cancelled','no_show'));

alter table reservation_demo.allocations
  drop constraint if exists allocations_status_check;

alter table reservation_demo.allocations
  add constraint allocations_status_check
  check (status in ('held','confirmed','checked_in','seated','completed','released','expired','cancelled','no_show'));
...
begin
  ...
  if v_table_code is not null then
    ...
    begin
      if v_member_count = 1 and v_active_allocation_count <= 1 then
        ...
        if not found then
          ...
          values (v_single_table_id, v_booking.id, v_booking.starts_at, v_booking.ends_at, v_status);
        end if;
      else
        update reservation_demo.allocations
        set status = 'released'
        ...
        insert into reservation_demo.allocations(table_id, booking_id, starts_at, ends_at, status)
        select table_id, v_booking.id, v_booking.starts_at, v_booking.ends_at, v_status
    ...
    update reservation_demo.allocations
    set status = case when v_status in ('cancelled','completed','no_show') then 'released' else v_status end
    where booking_id = v_booking.id
      and status <> 'released';
  end if;
    ...
    'bookings', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'visitCount', coalesce((
          ...
          where vb.guest_profile_id = gp.id and vb.status not in ('cancelled','no_show')
        ), 0),

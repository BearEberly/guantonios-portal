-- Public operator reset must remain reliable while SMS is disabled. PostgREST
-- execution currently raises cardinality_violation when the reset RPC updates
-- the sms_conversations parent row. Keep message/action tombstoning in the RPC
-- and rely on the private admin migration path for conversation-state cleanup.

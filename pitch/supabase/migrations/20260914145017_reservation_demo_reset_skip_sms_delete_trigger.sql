-- Prevent the protected demo reset from running SMS invalidation through the bookings DELETE trigger.
-- SMS state is intentionally preserved while synthetic booking rows are reset.
...
as $$
begin
  if tg_op <> 'TRUNCATE' and coalesce(current_setting('reservation_demo.reset_active', true), '') <> 'true' then
...
begin
  ...
  perform set_config('reservation_demo.reset_active', 'true', true);

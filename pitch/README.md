...
- `/operator` is a protected internal iPad view for check in, reservation detail edits, seating status changes, timed table availability blocks, manual service-stage tracking, cancellation, no-show marking, floor snapshot, Notify requests, waitlist, Texts readiness, recent holds, and reset.
...
The API regression covers reset, availability search, Notify creation and operator update, timed table block creation and blocked-table seating rejection, hold retry, confirm retry, wrong-token rejection, view, protected operator reservation detail edit, protected operator table assignment, change, cancel, no-show, operator list, one-table concurrency, and direct private schema isolation. The Playwright suite covers desktop, iPad sized, and phone sized booking and operator flows, plus automated accessibility smoke checks.

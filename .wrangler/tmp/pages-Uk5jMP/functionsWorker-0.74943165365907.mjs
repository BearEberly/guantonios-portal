...
// api/integrations/square/_shared.js
...
function resolveAdminRedirectUri(request, env, params) {
  ...
  entries.forEach(/* @__PURE__ */ __name(function appendPair(pair) {
    const key = pair[0];
    const value = pair[1];
...
function normalizeScopes(scopes) {
  if (Array.isArray(scopes)) {
    return scopes.map(/* @__PURE__ */ __name(function mapScope(item) {
      return String(item || "").trim();
    }, "mapScope")).filter(Boolean).join(" ");
  }
...
// api/integrations/square/connect.js
...
// api/integrations/square/disconnect.js
...
// api/integrations/square/status.js
...
// api/integrations/square/sync-day.js
...
function normalizeScope(scopes) {
  if (Array.isArray(scopes)) {
    return scopes.map(/* @__PURE__ */ __name(function mapScope(scope) {
...
function partsFromDate(date, timeZone) {
  ...
  const output = {};
  parts.forEach(/* @__PURE__ */ __name(function mapPart(part) {
...
function dateRangeForBusinessDay(dateIso, timeZone) {
  const [year, month, day] = dateIso.split("-").map(/* @__PURE__ */ __name(function toNumber(part) {
    return Number(part);
  }, "toNumber"));
  const nextDateIso = addDaysIso(dateIso, 1);
  const [nextYear, nextMonth, nextDay] = nextDateIso.split("-").map(/* @__PURE__ */ __name(function toNumber(part) {
    return Number(part);
  }, "toNumber"));
...
async function fetchSquareTips(input) {
  ...
  for (; ; ) {
    ...
    const payload = await response.json().catch(/* @__PURE__ */ __name(function fallbackJson() {
      return null;
    }, "fallbackJson"));
    if (!response.ok) {
      return {
    ...
    payments.forEach(/* @__PURE__ */ __name(function sumPayment(payment) {
...
async function fetchSquareOrderMetrics(input) {
  ...
  for (let start = 0; start < orderIds.length; start += chunkSize) {
    ...
    orders.forEach(/* @__PURE__ */ __name(function inspectOrder(order) {
      totals.orderCount += 1;
      ...
      if (serviceCharges.length) {
        serviceCharges.forEach(/* @__PURE__ */ __name(function inspectServiceCharge(serviceCharge) {
...
function extractSquareError2(payload) {
  ...
  if (Array.isArray(payload.errors) && payload.errors.length) {
    return payload.errors.map(/* @__PURE__ */ __name(function toMessage(errorItem) {
    ...
    }, "toMessage")).filter(Boolean).join(" | ");
  }
  return String(payload.error || payload.message || "");
}
__name(extractSquareError2, "extractSquareError");
...
__name(readBearerToken2, "readBearerToken");
...
__name(fetchCallerUser2, "fetchCallerUser");
...
__name(fetchCallerRole2, "fetchCallerRole");
...
__name(extractError2, "extractError");
...
__name(readBearerToken3, "readBearerToken");
...
__name(extractError3, "extractError");
...
__name(readBearerToken4, "readBearerToken");
...
__name(fetchCallerProfile2, "fetchCallerProfile");
...
__name(normalizeStoragePath2, "normalizeStoragePath");
...
__name(extractError4, "extractError");
...
// ../.wrangler/tmp/pages-Uk5jMP/functionsRoutes-0.38488379789096305.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-YrPVFW/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-YrPVFW/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.74943165365907.mjs.map
